...
;; any other structured language like Pascal. See sli-tools below.
...
[SPECIAL-HEAD-STRING special-head INDENT-SPECIAL-HEAD CLOSING-STRING]
This key is closed by CLOSING-STRING which belongs to `sli-separators'
...
HEAD-STRING's, STRONG-STRING's, MATH-RELATION-STRING's, BEACON-STRING's,
SEPARATORS and END-STRING's should all be different.
SOFT-KEY's are different from any of the above, but a same soft key can be
used in different constructs. Usual examples are 'then' and 'do'.
...
List of (cons STRING INDENTATION).")
...
(defvar sli-all-keys-regexp nil)
(defvar sli-all-end-strong-regexp nil)
(defvar sli-fixed-regexp nil)

(defvar sli-head-end-alist nil) ; ((cons end head) ...)
(defvar sli-end-head-alist nil) ; ((cons head (end1 end2 ...) ...)
(defvar sli-head-strong-alist nil) ; ((cons strong head) ...)
(defvar sli-companion-strong-keys-alist nil
"The car should be a member of the cdr if a strong.")
(defvar sli-soft-alist nil) ; ((cons ambiguous-soft . (head-or-strong1 head-or-strong2 ...)) ...)
(defvar sli-soft-head-or-strong-alist nil) ; ((cons head-or-strong soft) ...)
...
(defvar sli-special-head-alist nil) ;((cons special-head separator) ...)
...
(defvar sli-maid-ambiguous-soft nil
"List of keys that may ask for a different
following key according to context.
They *should be* 'soft' keys.
Subset of (mapcar 'car `sli-soft-alist').")
...
(mapcar 'make-variable-buffer-local
 '(sli-structures sli-shift-alist sli-separators sli-is-a-separatorp-fn
   ...
   sli-soft-head-or-strong-alist sli-head-end-alist sli-head-strong-alist
   sli-special-head-alist sli-end-head-alist sli-relevant-alist
   ...
   sli-maid-alist sli-maid-ambiguous-soft))
...
(defun sli-get-end-head-alist nil
...
(defun sli-get-head-strong-alist nil
  (let ((res '())) ; forme la liste (strong-key . head-key)
   ; Peut-etre plusieurs strong pour chaque head.
   (mapcar
     (lambda (ph)
       (if (equal (elt (elt ph 0) 1) 'head)
           (let ((strong (sli-get-strong (sli-flatten ph))))
              (unless (null strong)
                 (mapcar (lambda (st)
...
(defun sli-get-companion-alist nil ; case ?? It was not there.
  (let ((res '()))
    ; on prend les car de sli-head-strong-alist on leur
    ; associe la liste des car qui ont le meme cdr:
    (mapcar
      (lambda (co)
        (let ((end (cdr co)) (companions '()))
          (mapcar
            (lambda (coo)
               (when (equal (cdr coo) end)
                 (setq companions (add-to-list 'companions (car coo)))))
            sli-head-strong-alist)
          (setq res (append res (list (cons (car co) companions))))))
      sli-head-strong-alist)
    ; on prend les cdr de sli-head-strong-alist on leur
          ...
          (mapcar
            (lambda (coo)
               (when (equal (cdr coo) end)
                 (setq companions (add-to-list 'companions (car coo)))))
            sli-head-strong-alist)
          (setq res (add-to-list 'res (cons end companions)))))
      sli-head-strong-alist)
    res))
...
(defun sli-equivalence-classes-local (lst)
  (cond
   ...
   (t (let (lstbis (done nil) (l1 (car lst)))
	...
	(unless done
	  (setq lstbis (append lstbis (list l1))))  ; (print (append lst lstbis))
	lstbis))))
...
(defun sli-get-ancestors-alist nil
  (append
   ...
   (mapcar
    (lambda (strong)
      (cons strong
	    (append (list (cdr (assoc strong sli-head-strong-alist)))
...
(defun sli-get-maid-alist-locally (ph lst)
  (let ((res '()) aux resaux (nlst '()))  ; (print (list "In" ph lst))
    (cond
      ((null ph))
      ((listp (car ph))
...
;; sli-maid-ambiguous-soft is also created here.
  (setq sli-maid-ambiguous-soft nil)
  (let ((res '()))
    (mapcar
      (lambda (ph)
    ...
    (mapcar (lambda (co) (let ((to (sli-full-assoc co res)))
                           (cons co (if (null (cdr to)) (car to)
                                        (progn
                                          (add-to-list 'sli-maid-ambiguous-soft co)
					  to)))))
...
(defun sli-precomputations nil
  ...
  (setq sli-end-head-alist (sli-get-end-head-alist)
	sli-head-end-alist (sli-get-head-end-alist)
	sli-head-strong-alist (sli-get-head-strong-alist)
	...
	;; offsets :
        ...
        sli-maid-alist (sli-get-maid-alist) ; sli-maid-ambiguous-soft also is created there.
        )
  )
...
(defsubst sli-get-strong-from-strong-or-head (strong)
  (cdr (assoc strong sli-companion-strong-keys-alist)))

(defsubst sli-get-head-from-end (end)
  (cdr (assoc end sli-head-end-alist)))

(defsubst sli-get-head-from-strong (strong)
  (cdr (assoc strong sli-head-strong-alist)))

(defsubst sli-get-head-from (key)
  (cond
    ((member key sli-end-keys) (sli-get-head-from-end key))
    ((member key sli-strong-keys) (sli-get-head-from-strong key))))
    
(defsubst sli-get-end-from-head (head)
  (cdr (assoc head sli-end-head-alist)))

(defsubst sli-get-head-and-strong-from-soft (soft)
  (cdr (assoc soft sli-soft-alist)))

(defsubst sli-get-end-from-strong (strong)
  (sli-get-end-from-head (sli-get-head-from-strong strong)))
...
(defun sli-get-first-fixed-or-strong-or-end (pt)
...
(defun sli-reduce-skel (skel &optional full)
  ...
  (if (null skel) nil
   (let*((word (car skel)) end-lst strong-lst
         ...
         (skel (if full (sli-reduce-skel (cdr skel)) (cdr skel))))
     (cond
       ...
       ((member word sli-head-keys)
        ...
        (setq end-lst (sli-get-end-from-head word))
       ...
       ((member word sli-strong-keys)  ;(print "??")
        ...
        (setq end-lst (sli-get-end-from-strong word)
              strong-lst (sli-get-strong-from-strong-or-head word))
        (mapcar (lambda (s)
        ...
        (cond
          (found-endp
	    (while (and skel (not (member (car skel) end-lst)))
              (setq skel (cdr skel)))  ;(print "ooyes")
            (append (list word) skel))
...
(defun sli-find-matching-key (pt whatwewant relevant &optional givekey)
...
(defun sli-get-key-for-soft (pt soft)
  ; answer is (block-comment-start . point)
  ; if PT is within a multiline-comment.
  (save-excursion
    (goto-char pt)
    (let ((level-comment1 0) (foundp nil)
          word start (in-stringp nil) (case-fold-search nil)
          (possible-head-strong (sli-get-head-and-strong-from-soft soft))
          (aregexp
            (sli-regexp-opt
              (append sli-keys sli-comment-starts
                      (list "\"" block-comment-start block-comment-end)))))
      ;(print (list "Getting in sli-get-key-for-soft."))
      (while (and (not foundp) (not (bobp)))
        (if (sli-anchored-posix-search-backward aregexp nil 1)
          (cond
            ((string= (setq word (match-string-no-properties 0)) "\"")
		...
		(or
		 (sli-find-matching-key start (sli-get-head-from-end word) (sli-get-relevant word))
		 (point-min)))))
            ((member word possible-head-strong)
             (setq start (point))
             (unless (sli-in-one-line-comment)
                     (setq foundp t)))
            (t nil))
           ))
       ;(print (list "Out of sli-get-key-for-soft with " (if foundp (cons word (point)) nil)))
      (if foundp (cons word (point)) nil))))
...
which is not within a comment or a string."
  (save-excursion
    ...
    (let ((level-comment1 0) (level 0) (foundp nil)
           word (in-stringp nil)
           (aregexp (sli-regexp-opt
                     (append sli-comment-starts
                       (list "\"" block-comment-start block-comment-end
                             separator)))) )   ;"\\*/\\|/[*/]\\|[\";]"))
      ;(narrow-to-region (point) end)
      (while (and (not foundp) (< (point) end))
        (if (re-search-forward aregexp end 1)
          (cond
            ((string= (setq word (match-string-no-properties 0)) "\"")
             (if (= (preceding-char) ?\\)
                 (setq in-stringp t) ; it should already be.
...
Or on line after if AFTERP is t."
  ...
  (let*((pt (point)) wd-lst beg-str full-key appui opp
        (first-stuff (and (not afterp) (sli-get-first-fixed-or-strong-or-end pt))))
    ;(print (list "Getting in sli-tell-indent. first-stuff = " first-stuff afterp))
    (catch 'indent
    (if (and (not (null first-stuff))
             (setq opp (assoc (car first-stuff) sli-fixed-keys-alist)))
        (throw 'indent (eval (cdr opp))))
    (if (and first-stuff (not (string= (car first-stuff) block-comment-start)))
        ; line starts by a strong/end key:
      (progn
	(setq appui
    ...
    (if (and first-stuff (string= (car first-stuff) block-comment-start))
        ; PT is within a One/multi-line-comment.
        ;(print "Within one/multi-line-comment"). BEWARE if you uncomment ...
        (throw 'indent (current-indentation)))

    (unless afterp
      ; here line doesn't start by a strong/end key:
      (save-excursion
        (if (= -1 (forward-line -1))
        ...
        (end-of-line)
        (setq pt (point))))
    (setq first-stuff (sli-get-first-non-end-key pt))
...
(defun sli-electric-terminate-line (&optional beg)
  ...
  (if (sli-within-long-comment)
    (insert-char ?\n 1)
    (narrow-to-region (sli-get-safe-place) (point-max)) (print (list "Region: " (point-min) (point-max)))
...
`sli-more-maidp' is nil, this behaviour is reversed."
  ...
  (let ((full-key (sli-get-first-non-end-key (point) t)) (key nil) (head nil) smore)
    (sli-remove-trailing-spaces)
    ; Sort ambiguity arising from ambiguous-soft:
    (when (and full-key (member (car full-key) sli-maid-ambiguous-soft))
      (unless (member (setq head (car (sli-get-key-for-soft (point) (car full-key))))
                      sli-head-keys)
              (setq head (sli-get-head-from-strong head))))
    ...
    (cond
      ...
      ((and (member (car full-key) sli-special-head-keys)
       ...
       (insert key))
      (t (setq key (if head ;  completion of a soft difficult-key:
                       (car (sli-get-end-from-head head))
...
(defun sli-tools (struct shift sep sepp fixed safe keyn mkey comm)
