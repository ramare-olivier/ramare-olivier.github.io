...
/*    Modified: Thu Oct 28 17:56:20 2004
      Author:   Olivier Ramare
      Modification: Adaptation to mupad 2.5.2:
             linsert ---> listlib::insertAt
             fun ---> (()-> ... ---> proc
             types in IGetSeries
             toto[n]:=NIL does not destroy the nth element of list toto.
             We use the select function instead.
*/
proc()
begin
  Dichromate:=newDomain("Dichromate");
  Dichromate::info:= "Library Dichromate\
 Calculation of Dichromate/Tutte/Jones polynomial.";
  Dichromate::interface:= {hold(DiChromatic), hold(RectDiChromatic),
                           hold(RectTutte)};
...
Dichromate::IsGraph:=  proc(G)
                       begin
    bool(nops(G)=2 and type(G[1])=DOM_SET and type(G[2])=DOM_LIST
                   and _and(op(map(G[2],proc(ar)begin bool(nops(ar)=2)end)))
...
Dichromate::NormalForm:=
  ...
  begin
    ...
    tab:=table(Gordered[ind] = [ind, ind]$ind = 1..nops(G[1])); 
    G[1]:={ind $ind= 1..nops(G[1])}; 
...
Dichromate::ConnectedCompo:=
  ...
  begin
    ...
    list_graph:=[];
    while (nops(G[1])>0) do
      list_graph:=listlib::insertAt(list_graph,[NextCompo(G,{op(G[1],1)})],0);
                  /* linsert on old mupad */
...
Dichromate::DiChromatic:=
  ...
  begin
    if testargs() then
      /* args ! */
      fct:=proc(tt)begin [type(tt),nops(tt)] end;
      if args(0)<>1 then
    ...
    mult:=1;
    for aux in G[1] do
      //print("Select in ",table_incidence[aux],aux);
      //print("Found ",nops(select(table_incidence[aux],has,aux))," occurences.");
      mult:=mult*hold(B)^(nops(select(table_incidence[aux],has,aux)));
      table_incidence[aux]:=select(table_incidence[aux],_not@has,aux);
      //print("Remains ",table_incidence[aux]);
    end_for;
    /* Our graph doesn't have any loops left. */
    /* Repare G[2]:                           */
    G[2]:=select(G[2],proc(ar)begin bool(ar[1]<>ar[2])end);
    ...
    if ({type(DiChromateq),type(DiChromatev)}<>{DOM_IDENT}
      ...
      userinfo(0,"Something went wrong...",G)//,2^(nops(G[2])),pol)
    end_if;
  ...
  begin
    ...
    if (nops(G[1])=1)
	  ...
	  if (not alonep)
	    then break;
	  end_if;
	end_for;//print("is ",aux, "alone? ",alonep);
	if alonep
	...
	end_if;
      ...
      /* Repare G[2]: */
      G[2]:=select(G[2],proc(ar)begin
                                  _not(contains(lonely_ones,ar[1])
                                       or contains(lonely_ones,ar[2])) end);
	...
	aux:=G[2][1];
        m:=nops(select(G[2],_equal,aux));
        G[2]:=select(G[2],_not@_equal,aux);
...
/* Dichromate::MakeAll(2, [[1, 2]], TRUE) donne
   Dichromate::Chapeaux = [[1, [1, 1]], [2, [1, 2]]]
   Dichromate::DDD =
   Dichromate::TabChap = table([2, [1, 2]] = 2, [1, [1, 1]] = 1)
                         Donne le numero du chapeau.
   Dichromate::TabCompo = [[[1, 2]], [[1], [2]]]
                         Les composantes de chaque chapeau.
   Dichromate::TabKappa = [1,0]
   Dichromate::TabDiv = table(1 = [], 2 = [])
   Dichromate::One = array(1..2, 1..2, (1,1) = 1, (1,2) = 0, (2,1) = 1, (2,2) = 1)
                     La matrice des partition1 <= partition2
   Bon, pour ce qui de l'ordre des coordonnees...
   Dichromate::One[x,y]  c'est
      1_{Dichromate::Chapeaux[x] <= {Dichromate::Chapeaux[y]}
      et la premiere coordonnee est celle de la ligne,
      et la seconde celle de la colonne.
*/
Dichromate::MakeAll:=proc(r,RefEdges,linearp)
  ...
  begin
    ...
    if type(DiChromatev)=DOM_IDENT then
      ...
      end_if;
    end_if;
    Dichromate::TabChap:=table(Dichromate::Chapeaux[MdC]=MdC$MdC=1..dim);
    Dichromate::TabCompo:=[Composantes(Dichromate::Chapeaux[MdC])$MdC=1..dim];
    Dichromate::TabKappa:=[Kappa(Dichromate::Chapeaux[MdC])$MdC=1..dim];        
    Dichromate::DivChap:=table(MdC=[]$MdC=1..dim);
    //print(Dichromate::TabKappa);print(Dichromate::DivChap);
...
Dichromate::InitialVect:=proc(RefEdges)
  ...
  begin
    Part:=proc(Edges,r)
            local comp, done, comp_list, e, c, chap, x;
          begin 
            comp_list:=[];
            for e in Edges do
              done:=FALSE;
    ...
    r:= nops(Dichromate::Chapeaux[1][2]);//print(r,Dichromate::Chapeaux);
    SubMultiSet:=
       map(combinat::powerset({x$x=1..nops(RefEdges)}),
           proc(x) begin map([op(x)],proc(y)begin RefEdges[y] end) end);

    SousEdges:=[];
    for sous in SubMultiSet do //print(sous,r);
 ...
 begin
   return(Dichromate::Mat
          (1,nops(Dichromate::Chapeaux),
           [Dichromate::Coeff(indc,indc2)$indc2=1..nops(Dichromate::Chapeaux)]))
 end_proc:
...
Dichromate::Pi:=proc(s,RefEdges)
 ...
 begin
   ...
   Psi:=proc(vec) begin _plus(vec[wh]*(Dichromate::PsiLocal)(wh)
                              $wh=1..nops(Dichromate::Chapeaux)) end;
   vect:=fp::nest(Psi,s-1)(Dichromate::InitialVect(RefEdges));
   return(normal(_plus(vect[wh]*DiChromateq^(Dichromate::Chapeaux[wh][1])
                       $wh=1..nops(Dichromate::Chapeaux))))
 end_proc:
...
Dichromate::IGetSeries:=proc(RefEdges)
  local ResolvanteInverse,indc,dim,specoef,a,b;//linSolve;
  begin
    dim:=nops(Dichromate::Chapeaux);
    specoef:=proc(a,b)
             begin
               return(Dichromate::One[b,a]
                      -Dom::ExpressionField(normal, iszero)
                      (hold(X)*(Dichromate::GammaFlat(b,a)
                                *(DiChromatev+1)^(Dichromate::TabKappa[a]))))
             end;
    ResolvanteInverse:=Dichromate::Mat(dim,dim,specoef);
    userinfo(1,"Calcul de la serie.");
    return(
     normal(
      (Dichromate::Mat(1,dim,[DiChromateq^(Dichromate::Chapeaux[indc][1])$indc=1..dim])*
       //linalg::transpose(Dichromate::One)*
       linalg::matlinsolve(ResolvanteInverse,
...
Dichromate::RectDiChromatic:=proc(r,s,Flag)
  ...
  begin
    case args(0)
      of 1 do
        if testargs() and (not (type(r)= DOM_INT or Dichromate::IsGraph(r))) then
          ...
          if type(r)=DOM_INT then
            aux:=[r,[[aux,aux+1]$aux=1..(r-1)],hold(All),FALSE];break
          else
      ...
      of 2 do
        if type(r)=DOM_INT and type(s)=DOM_INT then
          aux:=[min(r,s),[[aux,aux+1]$aux=1..(min(r,s)-1)],
                max(r,s),FALSE];
        elif type(r)=DOM_INT and s=hold(All) then
          aux:=[r,[[aux,aux+1]$aux=1..(r-1)],hold(All),FALSE];
      ...
      of 3 do
        if type(Flag)=DOM_BOOL then
          if type(r)=DOM_INT and type(s)=DOM_INT then
            ...
            aux:=[r,[[aux,aux+1]$aux=1..(r-1)],hold(All),Flag];
          elif Dichromate::IsGraph(r) and (type(s)=DOM_INT or s=hold(All)) then
    ...
    if (s<> hold(All)) and (forcep or s<r^2) then
      ...
      WholeSeries:=Dichromate::IGetSeries(RefEdges);//print(WholeSeries);
      if s=hold(All) then
...
//Dichromate::RectDiChromatic([{1,2},[[1,2]]],1);
//Dichromate::DiChromatic([{1,2},[[1,2]]]);
/*
Dichromate::DiChromatic([{1,2,3,4},[[1,2],[1,4],[2,3],[3,4]]]);
Dichromate::DiChromatic([{1,2,3,4,5,6},[[1,2],[1,4],[2,3],[3,4],[5,6],[3,5],[4,6]]]);
*/
/*   That's all, folks ! ------------- 970 lines ----------------*/
