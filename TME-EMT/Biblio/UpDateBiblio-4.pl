...
# chmod 755 UpDateBiblio-4.pl to make it executable
# Use simply with ./UpDateBiblio-4.pl
...
sub make_booklet_texfile {
    my ($fhfile, $file, $fhbooklet, $filename, $fhtexfile);
    my @files;
    my ($line, $artdir, $dh, $title, $key, $secondline);
    $artdir = "../Articles";
...
";
	...
	open($fhtexfile, ">$filename") #Open file for writing and empty it.
        ...
        print $fhtexfile "File \\texttt{".$artdir."/".$file."}\n\n";
        while($line = readline($fhfile)){
            ## Section:
...
sub make_booklet {
    #Compiling, bibtexing, compiling, re compiling
    system("pdflatex $booklet_texfile>/dev/null; bibtex $booklet_texfile_nosuffix>/dev/null; pdflatex $booklet_texfile>/dev/null; pdflatex $booklet_texfile>/dev/null");
}
